            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">الصور</a></li>
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0"> صور الفندق - {{ $hotel->name }}</h4>
                        <a class="text-white" href="{{ route('dashboard.hotels.index') }}">
                            <button class="btn btn-primary text-white" style="margin-bottom: 37px;"> العودة الي قائمة الفنادق </button>
                        </a>
                    </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <form action="{{ route('dashboard.hotels.upload_multi_images') }}"
                                  class="dropzone"
                                  id="my-dropzone">
                                        ...
                                        @foreach($images as $image)
                                            <tr>
                                                ...
                                                <td class="ff_fileupload_actions">
                                                    <button class="ff_fileupload_remove_file remove_file" data-hotel_id="{{ $hotel_id }}" data-image="{{ $image }}" type="button" aria-label="Remove from list">
                                                    </button>
                                                </td>
                                            </tr>
                                        @endforeach
...
@section('script')
    ...
    <script>
            ...
            init: function() {
                ...
                this.on('success', function(file,response) {
                    ...
                    $('.ff_fileupload_uploads').prepend('<tr>\n' +
                        ...
                        '                                                </td>\n' +
                        '                                                <td class="ff_fileupload_actions">\n' +
                        '                                                    <button class="ff_fileupload_remove_file remove_file" type="button" data-hotel_id='+hotel_id+' data-image="'+response.image+'" aria-label="Remove from list">\n' +
                        '                                                    </button>\n' +
                        '                                                </td>\n' +
                        '                                            </tr>');
                });
        ...
        $(document).on('click','.remove_file',function(){
            ...
            $.ajax({
                method:'POST',
                url:"{{ route('dashboard.hotels.delete_image') }}",
                data:{
                    hotel_id:hotel_id,
                    image:image,
                    '_token':"{{ csrf_token() }}"
