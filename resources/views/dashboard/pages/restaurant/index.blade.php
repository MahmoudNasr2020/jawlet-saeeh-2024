        ...
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="card-header">
                    <h4 class="header-title">المطاعم</h4>
                ...
                <div class="card-body">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 37px;">
                            ...
                            <form action="{{ route('dashboard.restaurants.index') }}" method="get">
                                <div class="input-group ">
                                    <input type="text" class="form-control" name="search" onkeyup="searchTable()"
                                        placeholder="ابحث عن خدمة..." id="searchInput" aria-label="ابحث عن ماركة"
                        ...
                        <button class="btn btn-primary text-white">
                            <a class="text-white" href="{{ route('dashboard.restaurants.create') }}">اضافة مطعم</a>
                        </button>
                    </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($restaurants as $restaurant)
                                    <tr id="row-{{ $restaurant->id }}">
                                        ...
                                        <td>{{ $restaurant->prise }}</td>
                                        <td>{{ $restaurant->rating }}</td>
                                        <td>{{ $restaurant->service_fees }}</td>
                                        <td>{{ $restaurant->location }}</td>
                                        <td class="table-user">
                                            <img src="{{ $restaurant->getFirstMediaUrl('default') }}" alt="table-user"
                                                class="me-2 rounded-circle" style="width: 70px;height: 70px" />
                                        </td>
                                        <td>{{ $restaurant->created_at ? $restaurant->created_at->format('Y-m-d') : '' }}
                                        </td>
                                        <td>
                                            <a href="javascript:void(0);"
                                                onclick="confirmDelete('{{ route('dashboard.restaurants.destroy', $restaurant->id) }}','{{ $restaurant->id }}')"
                                                class="text-reset fs-16 px-1 text-danger">
                                                <i class="ri-delete-bin-2-line" style="color: red"></i>
                                            </a>

                                            <a href="{{ route('dashboard.restaurants.edit', $restaurant->id) }}"
                                                class="text-reset fs-16 px-1 text-primary">
